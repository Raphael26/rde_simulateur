...
class AuthState(rx.State):
        ...
        with rx.session() as session:
            ...
            if user and user.password == hashed_password:
                ...
                if to_dashboard == True:
                    ...
                    # FIXED: Use self.user_id instead of self.get_user_id
                    return ps.MultiStepFormState.save_simulation_result(user_id=self.user_id)
                    #return ps.MultiStepFormState.execute_string_function()
            else:
                self.login_error = "Identifiant ou mot de passe incorrect."
                self.is_logged_in = False
    ...
    # FIXED: Converted from @rx.var (computed var) to regular method
    # Computed vars are evaluated at BUILD time, causing database errors during deployment
    # This method should be called at RUNTIME when user_id is needed
    def fetch_user_id(self) -> Optional[int]:
        """
        Fetches the user_id from the database based on connected_user.
        Call this method at runtime when you need to ensure user_id is set.
