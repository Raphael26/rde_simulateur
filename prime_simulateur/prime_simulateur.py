...
from prime_simulateur.pages import (
    date_department,
    sector,
    typology,
    choice_pdf,
...
print(f"=== Supabase Debug ===")
print(f"SUPABASE_URL set: {bool(supabase_url)}")
if supabase_url:
    print(f"SUPABASE_URL value: {supabase_url[:40]}...")
else:
    print(f"SUPABASE_URL value: NOT SET")
...
try:
    if supabase_url and supabase_key:
        ...
        try:
            ...
            print(f"✅ Connected to Supabase storage! Found {len(buckets)} buckets")
            ...
            print(f"⚠️ Storage check error (may be normal): {storage_err}")
    else:
...
def read_file_from_bucket(
    ...
    if supabase_client is None:
        print(f"❌ Cannot read file: Supabase client not initialized")
        return None
    
    try:
        print(f"📁 Reading file from bucket: {bucket_name}/{file_path}")
        raw = supabase_client.storage \
            .from_(bucket_name) \
            .download(file_path)
        print(f"✅ File downloaded successfully: {len(raw)} bytes")
    except Exception as e:
        print(f"❌ Error downloading file {file_path}: {repr(e)}")
        return None
