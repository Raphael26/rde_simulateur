...
def display_linked_article(article):
    return rx.table.row(
        rx.table.cell(
            ...
            rx.text(
                article["pdf_file"].to(str).replace(".pdf", "").strip(),
                #on_click=pdf_preview_modal(article["pdf_file"])
            )
        ),
        rx.table.cell(
            rx.link(
...
@rx.page(route="/mpr-list", title="RDE Simulateur")
def page_mpr_list():
    content = rx.vstack(
        rx.vstack(
            rx.spacer(),
            rx.heading(
                "Liste des fiches éligibles à MPR",
                size="5",
                color="#0d242c"
            ),
            list_every_pdf(),
    ...
    return ps.multi_step_layout_no_stepper(content, 80)
